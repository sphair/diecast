...
{
	if(objects_map.find(gameobject->get_id()) != objects_map.end())
		return; // GameObject already tracked for replication

	objects_map[gameobject->get_id()]= gameobject;

	replicate_new_gameobject(gameobject);

//	log_event("Network", "Replicating gameobject %1 for player %2", gameobject->get_name(), player->get_user_id());
}

void ReplicatedContainer::remove_gameobject(const ServerGameObjectPtr &gameobject)
{
	auto it = objects_map.find(gameobject->get_id());
...
{
	for(auto &gameobject : objects_map)
	{
		gameobject.second->clear_dirty_tracked_properties();
...
void ReplicatedContainer::sync_dirty_gameobjects()
{
	for(auto &gameobject : objects_map)
	{
		...
		create_event.add_argument(gameobject.second->get_id());
		bool has_dirty_properties = false;

		auto dirty_properties = gameobject.second->get_dirty_tracked_properties();
